...
from bot.utils.keyboards import (
    referral_menu, referral_share_keyboard,
    referral_rewards_keyboard, back_button,
)
...
async def show_referral(update, context, user):
    ...
    next_ms  = _next_milestone(count)
    bar      = _progress_bar(count % 5, 5)
    remaining = next_ms - count

    if fa:
        txt = (f"📤 <b>دعوت از دوستان</b>\n\n"
    ...
    if update.callback_query:
        try:
            ...
            pass
    await safe_send_message(update.effective_user.id, txt, parse_mode=ParseMode.HTML, reply_markup=kb)
...
async def show_referral_history(query, user):
    ...
    await query.edit_message_text(txt, parse_mode=ParseMode.HTML,
        reply_markup=back_button(lang, "referral:menu"))
...
async def handle_redeem(query, user, pts_cost, reward):
    ...
    if (user.referral_points or 0) < pts_cost:
        ...
        await query.answer(t1, show_alert=True); return
    async with get_session() as s:
        from sqlalchemy import select
        ...
        if reward == "pro_week":       _extend(db, "pro", 7)
        elif reward == "pro_month":    _extend(db, "pro", 30)
        elif reward == "premium_month": _extend(db, "premium", 30)
        elif reward == "premium_3month": _extend(db, "premium", 90)
        user.plan = db.plan; user.subscription_expires_at = db.subscription_expires_at
...
def _extend(db, plan_str, days):
    ...
    base = db.subscription_expires_at if (db.subscription_expires_at and db.subscription_expires_at > now) else now
    db.plan = plan_str
...
async def cb_referral(update, context):
    ...
    if action == "menu":
        ...
        lang=user.language; fa=lang=="fa"
        await query.answer("امتیاز کافی ندارید." if fa else "Not enough points yet.", show_alert=True)
        ...
        t1 = "لینک شما (برای کپی روی آن نگه دارید):" if fa else "Your link (tap-and-hold to copy):"
